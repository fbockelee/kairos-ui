...
import { Fournisseur } from './../../../entities/fournisseur/fournisseur.model';
...
export class ContratfournisseurFormComponent extends Lov implements OnInit {
  ...
  public selectOffournisseur_itm: SelectItem[];

  constructor(
	private _listeService: ListeService,         // Pour gestion des listes
	private _fournisseurService: FournisseurService,         // Pour gestion des select
  ...
  initForm = () => {
    ...
    let optionsFournisseur: any = {params: [
						],
            	notPaged:true
			   };
		
	  this._fournisseurService.getAll(optionsFournisseur)
	    .subscribe(
	     (data: Fournisseur[]) => {
		    ...
		    for (let c of data) {
				     data_itm.push({ 
									  label: (c.raisonsociale), 
								 	  value: (c.fournisseurid)
								   });
			}
	     	this.selectOffournisseur_itm = data_itm;
	     },
	     error => {
					console.log('erreur');		
	    });
